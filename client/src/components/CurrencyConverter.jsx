...
const FALLBACK_RATES = {
  ...
  RON: 4.58, BGN: 1.79, RUB: 90.5, TRY: 30.8,
  ...
  BDT: 110, LKR: 325, THB: 35.1, VND: 24500, IDR: 15650,
  MYR: 4.65, PHP: 56.1, KRW: 1325, TWD: 31.7, CNY: 7.24,
  AED: 3.67, SAR: 3.75, QAR: 3.64, KWD: 0.307, BHD: 0.377,
  OMR: 0.385, JOD: 0.71, ILS: 3.65, UAH: 37.8,
};
...
export default function CurrencyConverter({ onPin }) {
  ...
  const swap = () => { setFrom(to); setTo(from); setResult(null); setPinned(false); };

  const handlePin = () => { onPin(`${from} → ${to}`); setPinned(true); };
  ...
  return (
    <div className="card">
      <div className="section-header">
        ...
        {status && (
          <span className={`badge badge-${status === "live" ? "live" : "cached"}`}>
            {status === "live" ? "Live rate" : "Approx. rate"}
          </span>
        )}
      </div>
